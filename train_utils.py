...
def update_last_checkpoint(checkpoint_dir: Path) -> Path:
    ...
    try:
        ...
        # Windows often blocks symlink creation without developer mode or elevated privileges.
...
def push_checkpoint_to_hub(
    ...
    *,
    ...
    token: str | None = None,
    path_in_repo: str = CHECKPOINTS_DIR,
):
    api = HfApi(token=token)
    repo_id = api.create_repo(repo_id=repo_id, private=private, exist_ok=True).repo_id
    checkpoint_path_in_repo = (Path(path_in_repo) / checkpoint_dir.name).as_posix()
    ...
    logging.info("Checkpoint %s pushed to %s under %s", checkpoint_dir.name, repo_id, checkpoint_path_in_repo)
    return commit_info
