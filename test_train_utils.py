...
def test_update_last_checkpoint(tmp_path):
    ...
    last_checkpoint = update_last_checkpoint(checkpoint)
    assert last_checkpoint.exists()
    if last_checkpoint.is_symlink():
        assert last_checkpoint.resolve() == checkpoint
    else:
        assert last_checkpoint.is_dir()
        assert (last_checkpoint / "marker.txt").read_text() == "ok"
...
@patch("lerobot.utils.train_utils.HfApi")
def test_push_checkpoint_to_hub(mock_hf_api, tmp_path):
    checkpoint_dir = tmp_path / "000010"
    checkpoint_dir.mkdir()

    api = mock_hf_api.return_value
    api.create_repo.return_value.repo_id = "owner/model"

    push_checkpoint_to_hub(checkpoint_dir, "owner/model", private=True)

    api.create_repo.assert_called_once_with(repo_id="owner/model", private=True, exist_ok=True)
    api.upload_folder.assert_called_once_with(
        repo_id="owner/model",
        repo_type="model",
        folder_path=checkpoint_dir,
        path_in_repo="checkpoints/000010",
        commit_message="Upload checkpoint 000010",
